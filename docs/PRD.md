# Proactive Agent：持续观察与异步行动 Agent

版本：v1.0，研究与实施交接稿
日期：2026-09-06，Asia/Shanghai
产品负责人：Jerry
交付性质：产品需求、技术决策、协议约束、调试设计与验收计划；本轮未实现、未启动屏幕采集、未调用付费模型。
工作名：Proactive Agent；最终产品名和仓库名不构成开工前置条件。
...
1. 新建独立包与独立桌面应用，尽量不受其他应用 代码结构约束。
...
- 不迁移其他产品的历史数据库、账号、计费、Gateway 或生产部署。
...
不从其他应用 工作树相对导入代码，不通过软链接依赖旧业务内部实现。起步可把几个 TS package 作为清晰模块，只有需要独立构建边界时再拆包，避免空壳层级。
...
- SQLite 备份、性能与保留测试只针对新应用测试库，不访问其他应用 或 Supabase 数据库。
...
- 凭证由新应用自己的系统凭据/进程配置提供，不复用或复制其他应用 secrets，不进入SQLite、导出或stdout。
