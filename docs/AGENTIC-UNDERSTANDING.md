...
工具包括 `list_actions`（应用 Bundle ID、窗口 ID、时间下界和分页）、`get_action_detail`、`get_action_image`、`get_action_ax`、`get_app_info`。图像工具返回图片内容块，保留采集时间及原图／当前标注图来源。未知或未来动作被拒绝；工具失败作为显式错误结果交给模型。Web 工具仅在配置 Key 后声明。
...
「观测」显示每一步消息、输出、工具调用与结果。图片用原动作及哈希引用，完整证据仍按 ID 查询。初始 Prompt 保留历史标题；多步 token 用量汇总计算。Agentic 请求不复用旧的单次理解缓存，以免忽略新增历史和外部网页变化；同批并发的相同请求仍共享在途结果。
