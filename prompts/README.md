...
- `understanding.ts`：`systemPrompt` 保存事实理解规则；`buildUserPrompt` 是实际发送的完整中文正文，集中注入 action、app_info、evidence、AX 文字、焦点、reading_context、screenshot、view。图片字节由调用方单独附加。
...
屏幕理解的规则与输入使用 XML 标签区分来源；动态变量通过 `xmlData` 转义后注入，不重复传入原始 AX 属性树。`description` 独立说明当前活动主题、对象和操作/选区；`detail` 保留关联正文原文及来源、范围和完整性，不设固定字数，不扩写缺失原文。硬件事件保留为事实依据，但不是摘要的唯一内容。详见 [XML 请求边界](../docs/UNDERSTANDING-XML.md)。
...
历史追踪保存当时实际使用的正文，不因修改默认文件变化。用户输入、截图、AX、工具结果属于动态数据，不写入本目录。历史 `action_title/action_detail` 由读取层兼容为 `description/detail`，不改写旧记录；新模型 Schema 仅接受新字段。

理解记录和追踪将实际传入模型的阅读证据独立保存为 `contextEvidence`，详情工具优先返回它并保留截断标记。它不是模型生成的 detail；模型省略或脱敏的文本不会被程序自动拼回回答，原文来源与生成描述保持可区分。

屏幕理解缓存包含最终渲染后的 User Prompt，修改模板会影响后续请求的缓存键。追踪同时保存模板变量 `promptInput`，工具从结构化证据取证，不从中文排版反解析数据；旧 JSON 输入记录仍兼容。设置编辑器目前覆盖角色规则，不覆盖源码里的 User Prompt 模板。Agentic 规则明确新输出契约优先于自定义模板中的旧字段说明，但不修改用户保存的模板文件。
